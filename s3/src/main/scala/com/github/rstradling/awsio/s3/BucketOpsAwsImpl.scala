...
/**
  ...
  */
class BucketOpsAwsImpl[F[_]](client: S3AsyncClient)(
    implicit F: ApplicativeError[F, Throwable],
    transform: CompletableFuture ~> F)
    extends BucketOps[F] {
  def create(
      createBucketRequest: CreateBucketRequest): F[CreateBucketResponse] = {
  ...
  def delete(
      deleteBucketRequest: DeleteBucketRequest): F[DeleteBucketResponse] = {
